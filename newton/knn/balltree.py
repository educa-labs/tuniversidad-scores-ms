...
def gower_distance(X_j, X_k, **kwargs):
    ...
    for col in kwargs['categorical']:
        distance += np.dot(kwargs['W_i'][kwargs['column_hash'][col]],
                           categorical_distance(X_j[kwargs['column_hash'][col]], X_k[kwargs['column_hash'][col]]))
    for col in kwargs['continuous']:
        distance += np.dot(kwargs['W_i'][kwargs['column_hash'][col]],
                           continuous_distance(X_j[kwargs['column_hash'][col]], X_k[kwargs['column_hash'][col]],
                                               kwargs['R_i'][kwargs['column_hash'][col]]))
    distance += kwargs['W_i'][11] * (1 - np.absolute(
        kwargs['similarity_matrix'][int(X_j[kwargs['column_hash']['ID']]), int(X_k[kwargs['column_hash']['ID']])]))
