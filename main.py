...
def get_predicition():
    ...
    if request.method == "GET":
        ...
        if prediction_model is not None:
            result = {"probability":1 - prediction_model.posterior_cdf(data["query_score"], np.array([data["query_day"], ])),"r_score":prediction_model.score(), "prediction":prediction_model.posterior_mean([data["query_day"], ])}
            status = 200
        else:
